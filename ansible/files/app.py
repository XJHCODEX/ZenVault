...
load_dotenv("/home/ubuntu/wallet_storagedb/.env")

# Initialize Flask app
app = Flask(__name__)

# replace wallet_storagedb with wallet location / name
...
def upload_file():
    """Handles file upload and stores it in the database with a limit of 10 files."""
    ...
    try:
        ...
        # If we're at or above 10 files, delete the oldest one
        if file_count >= 10:
            cursor.execute("""
                DELETE FROM files 
                WHERE file_name = (
